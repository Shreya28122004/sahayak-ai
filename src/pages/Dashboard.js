            ...
            <div className="container">
                ...
                {userProfile?.role === 'community' && (
                    <div>
                        <div className="stats-row">
                            <div className="stat-card">
                                <div className="stat-number">{needs.filter((n) => n.submittedBy === currentUser?.uid).length}</div>
                        ...
                        {needs.filter((n) => n.submittedBy === currentUser?.uid).length === 0 ? (
                            <div className="card" style={{ textAlign: 'center', padding: 40 }}>
                                <div style={{ fontSize: '3rem', marginBottom: 12 }}>📋</div>
                            ...
                            needs.filter((n) => n.submittedBy === currentUser?.uid).map((need) => (
                                <div
                                    key={need.id}
                                    className={`card need-card ${getPriorityClass(need.aiUrgency)}`}
                                    style={{ marginBottom: 16 }}
                                        ...
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
                                                ...
                                                <span style={{
                                                    ...
                                                    {need.status === 'completed' ? '✅ Resolved' : need.status === 'assigned' ? '🔄 In Progress' : '🔓 Open'}
                                                </span>
                                            </div>
                                            {need.aiExplanation && (
